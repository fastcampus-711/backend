...
public interface CommonPostRepository<T extends CommonPost> extends JpaRepository<T, Long> {
    ...
    // 게시판 별 + 검색어 조회
    Page<T> findByDtypeAndTitleContaining(String table, String keyword, Pageable pageable);

    // 게시판 별 + 분류 별 + 검색어 조회
    Page<T> findByDtypeAndCategoryIdAndTitleContaining(String dtype, Long CategoryId, String title, Pageable pageable);
